...
<div class="max-w-lg">
    ...
    <div class="card bg-base-100 shadow">
        <div class="card-body">
            <h1 class="card-title text-xl mb-4">Nuevo parámetro de control</h1>
            ...
            <form method="POST" action="{{ route('admin.parametros.guardar') }}" class="space-y-4">
                @csrf
                ...
                <div class="form-control">
                    ...
                    <select name="tipo_documento" class="select select-bordered @error('tipo_documento') select-error @enderror">
                        <option value="cedula" @selected(old('tipo_documento') === 'cedula')>Cédula de identidad</option>
                        <option value="licencia" @selected(old('tipo_documento') === 'licencia')>Licencia de conducir</option>
                    </select>
                </div>

                <div class="form-control">
                    <label class="label"><span class="label-text font-medium">Nombre del parámetro</span></label>
                    <input type="text" name="nombre_parametro" value="{{ old('nombre_parametro') }}" required
                           class="input input-bordered @error('nombre_parametro') input-error @enderror"
                           placeholder="Ej. numero_cedula, vigencia_cedula">
                    <div class="label"><span class="label-text-alt text-base-content/60">Usar formato snake_case (sin espacios)</span></div>
                </div>
                ...
                <div class="form-control">
                    <label class="label cursor-pointer justify-start gap-3">
                        ...
                        <input type="checkbox" name="es_bloqueante" value="1"
                               ...
                               @checked(old('es_bloqueante', true))>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="btn btn-primary flex-1">Crear parámetro</button>
                </div>
            </form>
        </div>
    </div>
